...
public class ItemiDao {

	private Logger logger = Logger.getLogger(ItemiDao.class);
	private String proxool_M="proxool.defaultDB";
	private String proxool_S="proxool.defaultDB_s0";	
	static ItemiDao  instance=new ItemiDao();
	public static  ItemiDao  getInstance(){
		return instance;
	}
	private ItemiDao(){
	...
	public int insert(Itemi vo){
		int result = 0;		
		if( null == vo ){
			return -1;
		}		
		try {
			SqlBuilder sql=new SqlBuilder("insert into itemi");			
	 ...
	 * @return	 
	 */
	public List<Itemi> find(Map<String, Object> paramsMap,int pageNo,int pageSize){
		List<Itemi> list = null;
		try{
			SqlBuilder sql=new SqlBuilder("SELECT * FROM itemi",paramsMap);					
			sql.appendWhereParam("id", "id=?");
			sql.appendWhereParam("itemIname", "itemIname=?");
			...
			list=VOKit.rs2BeanList(rs, Itemi.class);
		}catch (Exception e) {
			logger.error("", e);
		} 
		return list;
	...
	public Integer findCount(Map<String, Object> paramsMap){
		int count=0;
		try{
			SqlBuilder sql=new SqlBuilder("SELECT count(id) FROM itemi",paramsMap);					
			sql.appendWhereParam("id", "id=?");
			sql.appendWhereParam("itemIname", "itemIname=?");
			sql.appendWhereParam("userid", "userid=?");
			sql.appendWhereParam("createDate", "createDate=?");
	 ...
	 * @return	
	 */
	public Itemi findByPK(Object id) {
	 ...
	 * @author mengdz	 
	 */
	public Itemi findSingle(Map<String, Object> params) {
		List<Itemi> list=find(params, 1, 1);
	 ...
	 * @return	 
	 */
	public int update(Itemi vo) {
		int result = 0;		
		if( vo.getId()==null){
			return -1;
		}		
		try {
			SqlBuilder sql=new SqlBuilder("update itemi");				
	...
	public int update(Map<String, Object> paramsMap) {
		int result = 0;					
		try {
			SqlBuilder sql=new SqlBuilder("update itemi",paramsMap);						
			sql.appendSetParam("id_set", "id=?");
			sql.appendSetParam("itemIname_set", "itemIname=?");
	...
	public int delete(int id) {
		...
		try {
			SqlBuilder sql=new SqlBuilder("update itemi set status=-1 WHERE id=?");
			//按顺序添加sql参数
			sql.params.add(id);			
			result = DBFactory.getDBObject(proxool_M).update(sql);					
		}catch (Exception e) {
	...
	public int delete(String[] ids) {
		...
		try {
			SqlBuilder sql = new SqlBuilder("update itemi set status=-1 WHERE ");
			sql.append(" id in (");
			for (String id : ids) {
