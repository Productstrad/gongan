...
public class MembersDao {
	...
	private Logger logger = Logger.getLogger(MembersDao.class);
	private String proxool_M="proxool.defaultDB";
	private String proxool_S="proxool.defaultDB_s0";	
	static MembersDao  instance=new MembersDao();
	...
	public int insert(Members vo){
		...
		try {
			SqlBuilder sql=new SqlBuilder("insert into members");			
	 ...
	 * @return	 
	 */
	public List<Members> find(Map<String, Object> paramsMap,int pageNo,int pageSize){
		List<Members> list = null;
		try{
			SqlBuilder sql=new SqlBuilder("SELECT * FROM members",paramsMap);					
			sql.appendWhereParam("id", "id=?");
			sql.appendWhereParam("weixincode", "weixincode=?");
			sql.appendWhereParam("nickname", "nickname=?");
			sql.appendWhereParam("focusTime", "focusTime=?");
			sql.appendWhereParam("lastaccesstime", "lastaccesstime=?");
			...
			list=VOKit.rs2BeanList(rs, Members.class);
		}catch (Exception e) {
			logger.error("", e);
		} 
		return list;
	...
	public Integer findCount(Map<String, Object> paramsMap){
		int count=0;
		try{
			SqlBuilder sql=new SqlBuilder("SELECT count(id) FROM members",paramsMap);					
			sql.appendWhereParam("id", "id=?");
			sql.appendWhereParam("weixincode", "weixincode=?");
			sql.appendWhereParam("focusTime", "focusTime=?");
			sql.appendWhereParam("lastaccesstime", "lastaccesstime=?");
	 ...
	 * @return	
	 */
	public Members findByPK(Object id) {
	 ...
	 * @author mengdz	 
	 */
	public Members findSingle(Map<String, Object> params) {
		List<Members> list=find(params, 1, 1);
	...
	public int update(Members vo) {
		...
		try {
			SqlBuilder sql=new SqlBuilder("update members");				
	...
	public int update(Map<String, Object> paramsMap) {
		int result = 0;					
		try {
			SqlBuilder sql=new SqlBuilder("update members",paramsMap);						
			sql.appendSetParam("id_set", "id=?");
			sql.appendSetParam("weixincode_set", "weixincode=?");
			sql.appendSetParam("focusTime_set", "focusTime=?");
			sql.appendSetParam("lastaccesstime_set", "lastaccesstime=?");
	...
	public int delete(int id) {
		...
		try {
			SqlBuilder sql=new SqlBuilder("update members set status=-1 WHERE id=?");
			//按顺序添加sql参数
			sql.params.add(id);			
			result = DBFactory.getDBObject(proxool_M).update(sql);					
		}catch (Exception e) {
	...
	public int delete(String[] ids) {
		...
		try {
			SqlBuilder sql = new SqlBuilder("update members set status=-1 WHERE ");
			sql.append(" id in (");
			for (String id : ids) {
